...
def get_all_school_ids(api_url="https://api.cps.edu/schoolprofile/CPS/TypeaheadSchoolSearch"):
    """
    ...
    if response.status_code == 200:
        data = response.json()
        school_ids = {school["SchoolID"] for school in data}  # Extracting School IDs into a set
        return school_ids
    else:
...
def fetch_school_profiles(school_ids, api_base_url="https://api.cps.edu/schoolprofile/CPS/SingleSchoolProfile?SchoolID={SchoolID}"):
    """
    ...
    for school_id in school_ids:
        url = api_base_url.replace("{SchoolID}", str(school_id))  # Correct URL formatting
        ...
        if response.status_code == 200:
            ...
            if data:  # Ensure data is not empty
                school_profiles.append({
                    ...
                    "Latitude": float(data.get("AddressLatitude", 0)) if data.get("AddressLatitude") else None,
                    "Longitude": float(data.get("AddressLongitude", 0)) if data.get("AddressLongitude") else None,
                    "Student Count": int(data.get("StudentCount", 0)),
                    "Low Income Student Count": int(data.get("StudentCountLowIncome", 0)),
                    "Graduation Rate": float(data.get("GraduationRate", 0)) if data.get("GraduationRate") else None,
                    "Atttendance Rate Current Year": float(data.get("AttendanceRateCurrentYear", 0)) if data.get("AttendanceRateCurrentYear") else None,
                    "Address State": data.get("AddressState", "N/A"),  # Default to "N/A" if missing
                    ...
                    "Is Elementarty School": bool(data.get("IsElementarySchool", False)),
                    "Is Pre School": bool(data.get("IsPreSchool", False))
                })
        else:
            print(f" Error fetching data for School ID {school_id}: {response.status_code}")
