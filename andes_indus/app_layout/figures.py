...
def create_chicago_school_visualization(gdf_chicago, df_schools, selected_year):
    ...
    if not hasattr(df_schools, "geometry"):
        gdf_schools = gpd.GeoDataFrame(
            ...
            geometry=gpd.points_from_xy(
                df_schools["Longitude"], df_schools["Latitude"]
            ),
            crs="EPSG:4326",
        )
    else:
    ...
    brush = alt.selection_interval(
        encodings=["latitude"], empty=False, value={"latitude": [41.80, 41.90]}
    )
    ...
    chicago_map = (
        alt.Chart(chicago_data)
        .mark_geoshape(fill="lightgray", stroke="white", strokeWidth=0.1)
    ...
    left_map = (
        alt.layer(chicago_map, schools)
        .project(type="mercator")
        .properties(width=600, height=400)
    )

    # Ensure DropoutRate is numeric (if not already)
    gdf_schools["DropoutRate"] = pd.to_numeric(
        gdf_schools["DropoutRate"], errors="coerce"
    )
...
def create_stacked_chart_gender(df_c_long, selected_year):
    ...
    color_scale = alt.Scale(domain=["men", "women"], range=["#005A9C", "#A8D0E6"])

    indicator_order = ["Elementary", "Middle", "High School"]

    # -- Gender selection (bound to legend) --
    gender_selection = alt.selection_point(fields=["cut_name"], bind="legend")
    ...
    bar = (
        ...
        .encode(
            x=alt.X(
                "sum(value):Q",
                stack="zero",
                axis=alt.Axis(title="Population", titleFontSize=14, labelFontSize=12),
            ),
...
def create_stacked_chart_race(df_c_long, selected_year):
    ...
    df_filtered2 = df_c_long[
        ...
        & (df_c_long["indicator_label"].isin(["High School", "Middle", "Elementary"]))
        & (
            df_c_long["cut_name"].isin(
                ["hispanic", "afroamerican", "nonafroamerican_hispanic"]
            )
        )
    ]
    ...
    color_scale2 = alt.Scale(
        domain=[
            "afroamerican",
            "hispanic",
            "nonafroamerican_hispanic",
        ],  # The categories in cut_name
    ...
    race_selection = alt.selection_point(fields=["cut_name"], bind="legend")
    ...
    bar = (
        ...
        .add_params(race_selection, year_selection)
        .properties(
            width=500,  #  Increase width
            height=100,  # Increase height
...
def create_graph_multiple(df_c_long):
    ...
    df_filtered2 = df_c_long[
        (df_c_long["PUMA"] == 9999)
        & (
            df_c_long["indicator"].isin(
                [
                    "attendance_rate_elementary",
                    "attendance_rate_middle",
                    "attendance_rate_high",
                ]
            )
        )
        & (
            df_c_long["cut_name"].isin(
                ["afroamerican", "nonafroamerican_hispanic", "Total", "hispanic"]
            )
        )
    ]
    ...
    graph = (
        ...
        .encode(
            alt.X(
                "value",
                scale=alt.Scale(domain=[60, 100], zero=False),
                axis=alt.Axis(title="Percentage", titleFontSize=14, labelFontSize=12),
            ),
            y=alt.X(
                "year:N",
            ...
            color=alt.Color(
                "cut_name:N",
                legend=alt.Legend(title="Cut", titleFontSize=14, labelFontSize=12),
            ),
            facet=alt.Facet(
                "indicator_label:O", columns=1, title="", sort=indicator_order
            ),
        )
        .properties(
            width=450,
...
def create_scatter_dynamic(
    ...
    df_scatter = df[(df["year"] == selected_year) & (df["level"] == selected_level)]
    ...
    base = (
        ...
        .encode(
            ...
            y=alt.Y(
                "attendance_rate:Q",
                title=f"Attendance Rate - {level_labels[selected_level]}",
            ),
    ...
    chart = base.facet(column=alt.Column("race:N", title="Race"))

    regression = (
        alt.Chart(df_scatter)
        .transform_regression(f"{selected_crime}", "attendance_rate")
        .mark_line()
        .encode(x=f"{selected_crime}:Q", y="attendance_rate")
    )

    chart = (base + regression).facet(column="race:N")
    return chart
