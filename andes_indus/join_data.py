...
from merge_shp import load_pumas_shp, load_neighborhood_shp, gen_chi_bbox, gen_quadtree, load_schools, assign_division, School
import pandas as pd
...
def gen_final_data():
    ...
    crimes_by_puma = lower_colnames(grouped_data_by(crime_data, quadtree_chi_pumas, 'puma'))
    schools_by_puma = lower_colnames(grouped_data_by(schools_data, quadtree_chi_pumas, 'puma'))
    ...
    crimes_by_neighborhood = lower_colnames(grouped_data_by(crime_data, quadtree_chi_neighborhoods, 'neighborhood'))
    schools_by_neighborhood = lower_colnames(grouped_data_by(schools_data, quadtree_chi_neighborhoods, 'neighborhood'))
    ...
    data_neighborhoods = pd.merge(crimes_by_neighborhood, schools_by_neighborhood, how="inner", on=["neighborhood", 'year'])
    data_neighborhoods = pd.merge(data_neighborhoods, neighborhoods_shp, how="inner", on=["neighborhood"])
    print(data_neighborhoods.head())
